...
export class TurnOrderPanel {
  ...
  getDisplayObjects(): Array<Phaser.GameObjects.Image | Phaser.GameObjects.Rectangle | Phaser.GameObjects.Text> {
    return this.rows.flatMap((row) => [row.backing, row.border, row.avatar, row.teamMark, row.offsetText, row.badgeText]);
  }
  ...
  setLayout(config: {
    ...
    this.rowWidth = Math.max(112, config.avatarSize + 88);
    const rowHeight = config.avatarSize + 10;
    ...
    for (const [index, row] of this.rows.entries()) {
      ...
      row.badgeText.setPosition(leftX + 22, y + 10);
